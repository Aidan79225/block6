...
describe("Block", () => {
  it("creates a block with required fields", () => {
    const block = createBlock({
      id: "block-1", weekPlanId: "wp-1", dayOfWeek: 1, slot: 1,
      blockType: BlockType.Core, title: "專案開發", description: "完成 API 設計",
  ...
  it("rejects invalid dayOfWeek", () => {
    expect(() => createBlock({
      id: "block-1", weekPlanId: "wp-1", dayOfWeek: 0, slot: 1,
      blockType: BlockType.Core, title: "Test", description: "", status: BlockStatus.Planned,
    })).toThrow("dayOfWeek must be between 1 and 7");
  });

  it("rejects invalid slot", () => {
    expect(() => createBlock({
      id: "block-1", weekPlanId: "wp-1", dayOfWeek: 1, slot: 7,
      blockType: BlockType.Core, title: "Test", description: "", status: BlockStatus.Planned,
